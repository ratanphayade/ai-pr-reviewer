...
import {context as github_context, github} from '@actions/github'
...
const allowedEvent = 'issue_comment'
...
export const onDemandCodeReview = async (
  lightBot: Bot,
  heavyBot: Bot,
  options: Options,
  prompts: Prompts
  ...
  if (context.eventName !== allowedEvent) {
    warning(
      `Skipped: current event is ${context.eventName}, only support ${allowedEvent} event`
    )
    return
  }

  console.log(github.rest.repos.listPullRequestsAssociatedWithCommit);
